...
export function StatsBar() {
  ...
  <section className="w-full text-white pt-16 md:pt-20">
      <div ref={ref} className="max-w-6xl mx-auto px-6">
        ...
        {error && (
          <div className="mt-6 text-center text-xs text-red-500/70">GitHub stats could not load (rate limit or network). Showing fallback.</div>
        )}
      </div>
    </section>
  )
...
function Stat({ item, index, inView, loading }: { item: StatItem; index: number; inView: boolean; loading?: boolean }) {
