...
const services: ServiceItem[] = [
  ...
  details: 'Reduced decision fatigue for coaches & analysts by layering metrics into digestible tiers. In DeepSport Dashboard separated live sports measurements into meaningful layers with minimal latency; implemented progressive loading / skeleton strategies preserving readability and interaction under heavy usage.',
    bullets: [
  ...
  },
  {
    ...
    icon: Boxes,
  description: 'Safe evolution aligned with product growth: modular monolith → targeted service extraction.',
    stack: ['Domain Design', 'Queues', 'CI/CD', 'Observability'],
    gradient: 'from-amber-500/20 to-orange-500/20',
  details: 'Motivation: avoid premature microservice complexity; keep growth controlled, observable, flexible. Start with clear domain boundaries in a single codebase, then isolate data & transfer hot paths for phased extraction. Emphasis on observability, resilience and maintainability over accidental complexity.',
    bullets: [
              ...
              <div className="relative h-full rounded-2xl bg-white dark:bg-neutral-950 p-6 md:p-8 flex flex-col shadow-[0_0_0_1px_rgba(255,255,255,0.05)] min-h-[420px]">
                ...
                <div className="absolute top-4 right-4 z-10">
                  <div className="text-[10px] font-medium tracking-wide px-3 py-1.5 rounded-full bg-neutral-900/70 dark:bg-white/10 text-white/90 dark:text-white backdrop-blur-md shadow-lg border border-white/10 flex items-center gap-1 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition duration-300">
                    <span>Learn</span><span className="text-white/60">More</span>
                  </div>
                </div>
                <div className={`relative w-14 h-14 rounded-xl bg-gradient-to-br ${s.gradient} flex items-center justify-center mb-6 transition-transform duration-500 group-hover:scale-105 group-hover:rotate-[1.5deg]`}>
                ...
                <h3 className="relative text-xl font-semibold mb-4 tracking-tight flex items-center gap-2" role="heading" aria-level={3}>
                  {s.title}
                </h3>
                <p className="relative text-sm text-gray-600 dark:text-gray-300 leading-relaxed mb-6 flex-1 max-w-[260px]">{s.description}</p>
                <div className="relative flex flex-wrap gap-2 mt-auto">
                  {s.stack.map(tag => (
                    <span key={tag} className="px-3 py-1 text-xs rounded-full bg-black/2 dark:bg-white/5 border border-black/5 dark:border-white/5 text-gray-700 dark:text-gray-300 backdrop-blur-[2px]">
                      {tag}
                    </span>
                  ))}
                </div>
